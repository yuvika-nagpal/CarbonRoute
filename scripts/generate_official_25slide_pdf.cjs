...
const fs = require('fs');
const { PDFDocument, rgb, StandardFonts } = require('../backend/node_modules/pdf-lib');
...
async function generate25SlidePDF() {
  ...
  for (let i = 0; i < slidesData.length; i++) {
    ...
    if (s.isArch) {
      ...
      drawBox('Public Website', 120, height - 140, 200, 42);
      drawBox('Frontend Dashboard', 380, height - 140, 200, 42);
      drawBox('FastAPI Backend', 640, height - 140, 200, 42);

      drawBox('Scheduling', 120, height - 220, 200, 42);
      drawBox('Risk / Calibration', 380, height - 220, 200, 42);
      drawBox('Experiment Engine', 640, height - 220, 200, 42);

      drawBox('PostgreSQL', 240, height - 300, 220, 42);
      drawBox('Object Storage', 500, height - 300, 220, 42);

      drawBox('Simulator / Connector', 340, height - 380, 280, 42);
